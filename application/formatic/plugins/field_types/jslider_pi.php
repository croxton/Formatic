...
class Jslider extends Formatic_plugin {
	...
	function run($f, $param)
	{
		...
		/*
		$formatic->load_asset('css', array($config['skin_dir'].'jslider.'.$config['skin'].'.css', 'screen'));
		
		// asset for IE6 support
		$formatic->load_asset('css', array($config['skin_dir'].'jslider.'.$config['skin'].'.css', 'screen', 'IE'));
		*/
